      ...
      <div class="container mx-auto px-6 py-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-lg font-bold text-white">Airfield Simulation</h1>
              <p class="text-xs text-slate-400">Hangar Bay Scheduling</p>
            </div>
          </div>
          <a href="#login" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium rounded-lg transition-colors border border-slate-700 hover:border-slate-600 flex items-center gap-2">
    ...
    <div class="relative overflow-hidden">
      ...
      <div class="relative container mx-auto px-6 py-16 text-center">
        ...
        <p class="text-lg text-slate-400 max-w-2xl mx-auto mb-8">
          Model, validate, and schedule aircraft hangar bay allocations using our domain-specific language.
          Select an example below or write your own DSL code.
        </p>
...
function createExamplesSection(): string {
  ...
  return `
    <section class="mb-12">
      <h2 class="text-2xl font-bold text-white mb-6">Select an Example</h2>
...
function createExampleCard(example: Example, category: { key: string, label: string, icon: string, color: string }): string {
  ...
  return `
    <button
      ...
      class="example-card group relative p-5 rounded-xl border ${colors.border} ${colors.bg} text-left transition-all duration-200 hover:scale-[1.02] hover:shadow-lg hover:shadow-${category.color}-500/10"
    >
      <div class="flex items-start gap-3">
        <div class="w-10 h-10 rounded-lg ${colors.bg} border ${colors.border} flex items-center justify-center flex-shrink-0">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            <h3 class="font-semibold text-white truncate">${example.name}</h3>
          </div>
          <p class="text-sm text-slate-400 line-clamp-2">${example.description}</p>
  ...
  return `
    <section class="mb-8">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-2xl font-bold text-white">DSL Editor</h2>

...
function mkManualInduction(
    ...
    return {
        ...
        hangar:   { ref: hangar,   $refText: hangar?.name   ?? '' },
        bays: bays.map((b: any) => ({ ref: b, $refText: b?.name ?? '' })),
        door: door ? { ref: door, $refText: door?.name ?? '' } : undefined,
        start,
        end,
        clearance: undefined,
        $type: 'Induction'
...
function mkAutoInduction(
    ...
    duration: number,
    options: { notBefore?: string; notAfter?: string } = {}
) {
    return {
        id,
        aircraft: { ref: aircraft, $refText: aircraft?.name ?? '' },
